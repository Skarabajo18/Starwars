...
def app(request):
    ...
    return render(
        ...
        request,
        'app.html',
        context={'num_nave': num_nave, 'num_pilotos': num_pilotos,
                 'num_modelo': num_modelo, 'num_dueñoN': num_dueñoN},
    )
...
def contacto(request):
    if request.method == 'POST':  # Si el formulario es enviado#
        ...
        if form.is_valid():  # Si es valido se procesan los datos
            # y se redirige a la url
            return HttpResponseRedirect('GestionNaves/templates/')
    else:
